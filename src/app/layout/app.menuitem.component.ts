			...
			<a *ngIf="(!item.routerLink || item.items) && item.visible !== false" [attr.href]="item.url" (click)="itemClick($event)"
			   [ngClass]="item.class" [attr.target]="item.target" tabindex="0" pRipple>
				<ng-container *ngIf="item.icon">
                    <img *ngIf="isImagePath(item.icon)" [src]="item.icon" class="layout-menuitem-icon" alt="menu icon">
                    <i *ngIf="!isImagePath(item.icon)" [ngClass]="item.icon" class="layout-menuitem-icon"></i>
                </ng-container>
